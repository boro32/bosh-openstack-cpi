...
import (
	...
	bocapi "github.com/frodenas/bosh-openstack-cpi/api"
)
...
type JSON struct {
	actionFactory bocaction.Factory
	caller        Caller
...
func (c JSON) buildCloudError(err error) []byte {
	...
	if typedErr, ok := err.(bocapi.CloudError); ok {
	...
	if typedErr, ok := err.(bocapi.RetryableError); ok {
